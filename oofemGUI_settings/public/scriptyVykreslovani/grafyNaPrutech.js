...
class vykresliKonstrukci {
    ...
    getPopisGrafu(){
        return(this.popisGrafyVsechnyPruty);
    }
    ...
    ZiskejDataProCanvas(id, elementId, vykresliCisloPrutu, poradnicePopis, zvyraznitCisloPrutu, zobrazCislaUzlu, zvyrazniCisloUzlu){
        ...
        console.log("pocetIdpocetIdpocetIdpocetIdpocetId");
    ...
    vykresliVsechnyKotyDaneKotyId(id, ctx, obj, vykresliCisloPrutu, poradnicePopis, zvyraznitCisloPrutu, zobrazCislaUzlu, zvyrazniCisloUzlu){
        ...
        for (var i = prvniIndexPrutu; i < posledniIndexPrutu; i++) {
            ...
            if(i == zvyraznitCisloPrutu){
                ...
                var popisGrafyPrut = prut.getPopisGrafyPrut();
                this.popisGrafyVsechnyPruty.push(popisGrafyPrut);
...
class vykresliPrut {
    ...
    //vraci souradnice textu
    getPopisGrafyPrut(){
        return(this.popisGrafyPrut);
    }
    ...
    hlavniMetoda(Ax, Ay, Bx, By, koeficientyPredMocninamiFunkce, delkaKrokuPriblizne, ctx, Ox, Oy, barvaCaryPrut, tloustkaCaryPrut, barvaCarySrafy, tloustkaCarySrafy, barvaCaryHranice, tloustkaCaryHranice, vykreslitGraf, meritkoGraf, poradnicePopis, uzelStart, uzelEnd, zvrazniUzelStart, zvyrazniUzelEnd){
        ...
        if(delkaKrokuPriblizne > 0){
            ...
            this.vykreslujHraniciGrafu(this.funkcniHodnotyXVekt, this.funkcniHodnotyYVekt, this.hraniceGrafuVykresleni, this.poradnicePopis);
    ...
    vykreslujHraniciGrafu(funkcniHodnotyXVekt, funkcniHodnotyYVekt, hraniceGrafuVykresleni, poradnicePopis)
    {
        ...
        this.zapisTextDoDat(poradnicePopis, funkcniHodnotyXVekt, funkcniHodnotyYVekt)

    }


    //zapise text do dat urcenych pro WS
    zapisTextDoDat(poradnicePopis, funkcniHodnotyXVekt, funkcniHodnotyYVekt){

        var delkaPole = funkcniHodnotyXVekt.length;

        //popise hodnoty na grafu na zacatku i na konci
        if(poradnicePopis == -1){

            //popis na zacatku
            var Bx = funkcniHodnotyXVekt[1]-5;  //text posunut o 5 pixelu
            var By = funkcniHodnotyYVekt[1]-5;
            ...
            if(hodnotaNaZacatku != 0){
                ...
                if(this.posouvejText == "true"){
                    this.pridejPopis(hodnotaNaZacatku, Bx, By);
                }
            ...
            //popis na konci
            var Bx = funkcniHodnotyXVekt[delkaPole-1]-5;
            var By = funkcniHodnotyYVekt[delkaPole-1]-5;
            var hodnotaNaKonci;
            ...
            if(hodnotaNaKonci != 0){
                ...
                //pokud se ma posouvat text, vykreslovani probiha jinde
                if(this.posouvejText == "true"){
                    this.pridejPopis(hodnotaNaKonci, Bx, By);
                }
                else {  //vykresluje se popis zde, neni treba nic posouvat
                    this.pridejPopis(hodnotaNaKonci, Bx, By);
                }
            ...
            var Bx = funkcniHodnotyXVekt[poradnicePopis];
            var By = funkcniHodnotyYVekt[poradnicePopis];
    ...
    pridejPopis(text, Ax, Ay){
        ...
        this.popisGrafyPrut.push(popis);
...
export const grafyNaPrutech = ((vykresliJSON) => {
    ...
    console.log("DBGrafyNaPrutechDataDBGrafyNaPrutechDataDBGrafyNaPrutechDataDBGrafyNaPrutechData")
    console.log(vykresliJSON)
    console.log("DBGrafyNaPrutechDataDBGrafyNaPrutechDataDBGrafyNaPrutechDataDBGrafyNaPrutechData")
    ...
    var popisGrafyPrut = grafika.getPopisGrafu();
